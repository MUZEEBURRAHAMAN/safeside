...
| **Project skill pack** (`.agents/skills/`, 34 skills, committed to repo) | Auto-triggered while coding | Installed via Vercel skills CLI (`npx skills add …`; lockfile `skills-lock.json`). **UI feel:** `make-interfaces-feel-better` (polish gate: micro-interactions, optical alignment, tabular numbers for score/meter digits, enter/exit animation). **SwiftUI build:** `swiftui-patterns/-layout-components/-navigation/-animation/-gestures/-performance/-uikit-interop/-liquid-glass`. **Swift quality:** `swift-language/-concurrency/-architecture/-api-design-guidelines/-codable/-formatstyle/-testing/-security`, `swiftlint`. **Domain:** `vision-framework` (OCR/barcode), `photokit` (gallery scan), `permissionkit` + `push-notifications` (priming), `ios-networking` (offline, Chunk 6), `ios-accessibility` (Chunk 6 sweep), `ios-simulator` (screenshot matrix), `debugging-instruments`, `ios-localization` (string expansion), `tipkit` (feature hints, use sparingly — never nagging), `background-processing`, `swift-charts` (Plan tab later). **Phase D shelf:** `storekit`, `authentication`, `app-store-optimization`, `app-store-review`. Pruned from 85 → 34 (carplay/gamekit/weatherkit etc. removed; re-add anytime with `npx skills add dpearson2699/swift-ios-skills@<name>`). |
